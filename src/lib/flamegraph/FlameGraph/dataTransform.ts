import { type DataFrame, type Field, FieldType, getFieldByName } from '../data';
import { type DisplayProcessor, getDisplayProcessor } from '../format';
...
export function nestedSetToLevels(
  ...
  for (let i = 0; i < container.data.length; i++) {
    ...
    const newItem: LevelItem = {
      ...
      valueRight: container.isDiffFlamegraph()
        ? container.getValueRight(i)
        : undefined,
      start: offset,
...
export function checkFields(data: DataFrame): CheckFieldsResult | undefined {
  ...
  for (const field of fields) {
    ...
    const frameField = getFieldByName(data, name);
    if (!frameField) {
...
export class FlameGraphDataContainer {
  ...
  constructor(data: DataFrame, options: Options, theme: Theme = getTheme()) {
    this.data = data;
    ...
    this.labelField = getFieldByName(data, 'label')!;
    this.levelField = getFieldByName(data, 'level')!;
    this.valueField = getFieldByName(data, 'value')!;
    this.selfField = getFieldByName(data, 'self')!;
    ...
    // Upstream also supports a label field backed by an enum config, read
    // through a display processor. Our data layer has no enum field type, so
    // labels are always plain strings.
...
function fieldAccessor(field: Field | undefined, index: number | number[]) {
  ...
  return indexArray.reduce<number>((acc, index) => {
