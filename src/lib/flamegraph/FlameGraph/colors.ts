...
export function getBarColorByPackage(label: string, theme: Theme) {
...
// green to red
export const diffDefaultColors = [
  'rgb(0, 170, 0)',
  'rgb(148, 142, 142)',
  'rgb(200, 0, 0)',
];
...
export const diffColorBlindColors = [
  'rgb(26, 133, 255)',
  'rgb(148, 142, 142)',
  'rgb(220, 50, 32)',
];
...
/**
 * Piecewise-linear RGB colour scale, equivalent to d3's
 * `scaleLinear().domain(domain).range(range)` when the range holds colours:
 * each channel is interpolated linearly, the scale is unclamped so values
 * outside the domain extrapolate, and channels are rounded then clamped to
 * 0-255 only when the colour is serialised.
 */
function rgbScale(domain: number[], range: string[]) {
  const stops = range.map((c) => color(c).toRgb());

  return (x: number): string => {
    // Pick the segment the way d3 does: the first segment covers everything
    // below the second domain stop, and each later stop takes over at its own
    // value, so the last segment also handles anything past the domain.
    let i = 0;
    while (i < domain.length - 2 && x >= domain[i + 1]) {
      i++;
    }
    ...
    const channel = (a: number, b: number) =>
      Math.max(0, Math.min(255, Math.round(a + t * (b - a)) || 0));
...
export function getBarColorByDiff(
  ...
  const range =
    colorScheme === ColorSchemeDiff.Default
      ? diffDefaultColors
      : diffColorBlindColors;
  const colorScale = rgbScale([-100, 0, 100], range);
