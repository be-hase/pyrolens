...
const FlameGraphCanvas = ({
  ...
  useFlameRender({
    ...
    textAlign,
    totalViewTicks,
    // We need this so that if we have a diff profile and are in sandwich view
    // we still show the same diff colors.
    totalColorTicks: data.isDiffFlamegraph()
      ? totalProfileTicks
      : totalViewTicks,
  ...
  return (
    <div className="plfg-canvas-graph">
      <div
        className="plfg-canvas-wrapper"
  ...
  while (next) {
    ...
    for (const child of nextList) {
      ...
      if (xStart <= pos.x && pos.x < xEnd) {
        next = child;
        // Check if item is a collapsed item. if so also check if the item is the
        // first collapsed item in the chain, which we render, or a child which
        // we don't render. If it's a child in the chain then don't increase the
