...
`slowRegression` frame tops the ranking at `+38%`, red in the flame graph.
Because the diff is one query over both windows, Diff has a single Run
above the panes that commits both edits at once, instead of a Run per pane.
