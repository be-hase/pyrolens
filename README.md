...
| `maxNodes`                                          | positive integer (1–1,000,000) capping the flamegraph node count per query; server default when absent or invalid — a useful dial when heavy profiles are slow; adjustable from the Max nodes slider in the controls bar on the flame-graph views (Single, Comparison, Diff) |
